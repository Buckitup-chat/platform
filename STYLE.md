...
### Prefer `with` Over Nested `if`/`case`

Refactor nested `if`/`case` blocks into `with` for cleaner control flow:

```elixir
# Avoid nested if/case
def cleanup do
  if File.dir?(dir) do
    case File.ls(dir) do
...
with true <- condition1,
     {:ok, result} <- operation() do
  result
else
  false -> :error  # Which condition returned false?
...
with {:is_dir, true} <- {:is_dir, File.dir?(shm_dir)},
  ...
  {:is_dir, false} -> :ok
  {:error, reason} -> log(["Could not list: ", inspect(reason)], :debug)
end
```
